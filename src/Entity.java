public class Entity extends SceneNode
{
		...
		public final void updateHitData(int hitsplatColour, int damage, int clientCycle, int icon, int soak)
		{
				...
				{
						if (hitsLoopCycle[i1] <= clientCycle)
						{
								hitIcon[i1] = icon;
								hitmarkMove[i1] = 5;
								moveTimer[i1] = 2;
								...
								hitsLoopCycle[i1] = clientCycle + 70;
								hitSplatAppliedTime[i1] = System.currentTimeMillis();
								hitMarkPos[i1] = 0;
								return;
						}
				}
				...
				for (int i1 = 0; i1 < 4; i1++)
				{
						if (hitsLoopCycle[i1] < lowest)
						{
								lowest = hitsLoopCycle[i1];
								index = i1;
						}
				}
				hitIcon[index] = icon;
				...
				hitsLoopCycle[index] = clientCycle + 70;
				hitSplatAppliedTime[index] = System.currentTimeMillis();
				hitMarkPos[index] = 0;
		}
		...
		{
				if (entity.moveStartCycle == Client.loopCycle || entity.anim == -1 || entity.sequenceDelayCycle != 0 || entity.sequenceCycle + 1 > Animation.anims[entity.anim].getDuration(entity.sequenceFrame))
				{
		...
		private static void handleEntityMovement(Entity entity)
		{
				...
				if (entity.anim != -1 && entity.sequenceDelayCycle == 0)
				{
		...
		private static int[] gfxAnimationFixList = {7074, 2414, 10505, 7073, 4947, 4973};
		...
		public static void handleEntitySequences(Entity entity)
		{
				...
				{
						if (entity.moveSequence > Animation.anims.length)
						{
								entity.moveSequence = 0;
						}
						Animation animation = Animation.anims[entity.moveSequence];
						if (entity.moveSequenceFrame < animation.length && entity.moveSequenceCycle > animation.getDuration(entity.moveSequenceFrame))
						{
								...
								{
										if (entity.moveSequence < Animation.length474)
										{
												entity.moveSequenceCycle = 0;
										}
										else
										{
				...
				if (entity.spotanim != -1 && Client.loopCycle >= entity.spotanimEndCycle)
				{
						...
						Animation animation_1 = SpotAnim.cache[entity.spotanim].aAnimation_407;
						...
						entity.nextGraphicsAnimationFrame = entity.spotanimFrame + (cancelGfxAnimations(entity.anim) ? 0 : 1); // It was +1 which caused Ags Special attack bug, statius warhammer bug and air guitar emote bug.
				...
				if (entity.anim != -1 && entity.sequenceDelayCycle <= 1)
				{
