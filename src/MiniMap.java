public class MiniMap
{
		...
		public static void drawMinimap(Client client)
		{
				...
				int xOffset = Client.isFixedScreen() ? 0 : Client.getClientWidth() - 210;
				...
				// There used to be if (client.minimapState == 2) { return } check other clients, this is for barrows underground on other servers
				...
				client.landImage.method352(152, i, one, 256 + client.minimapZoom, two, l2, mapY, mapX, 146, j);
				for (int j5 = 0; j5 < client.objectIconCount; j5++)
				{
						try
						{
								...
								{
										int x = ((mapIconLocationsRemoved[index][0] - Client.instance.baseX) * 4 + 2) - Client.myPlayer.x / 32;
										int y = ((mapIconLocationsRemoved[index][1] - Client.instance.baseY) * 4 + 2) - Client.myPlayer.y / 32;
										if (xPosition == x && i3 == y)
										{
												match = true;
										}
				...
				{
						int x = ((customMapIconLocations[index][1] - Client.instance.baseX) * 4 + 2) - Client.myPlayer.x / 32;
						int y = ((customMapIconLocations[index][2] - Client.instance.baseY) * 4 + 2) - Client.myPlayer.y / 32;
				...
				for (int j6 = 0; j6 < client.playerCount; j6++)
				{
						...
						if (player != null)
						{
								if (player.gameModeTitle.contains("[Bot]") && Client.botsOff && (System.currentTimeMillis() - player.lastCombatTime) >= 5000)
								{
										continue;
								}
						}
				...
				{
						if (client.markType == 1 && client.markedNpc >= 0 && client.markedNpc < client.npcArray.length)
						{
						...
						if (client.markType == 10 && client.markedPlayer >= 0 && client.markedPlayer < client.playerArray.length)
						{

								int x = ((Client.targetHintX - Client.instance.baseX) * 4 + 2) - Client.myPlayer.x / 32;//Set x and Y cord for display area, or just use method 254
				...
				if (client.destX != 0)
				{
						...
						markMinimap(client, client.mapFlag, j2 + (Client.is474GameFrame && Client.isFixedScreen() ? -1 : -1), l4 + (Client.is474GameFrame && Client.isFixedScreen() ? 0 : 0));
				}
				if (Client.is474GameFrame && Client.isFixedScreen())
				{
						Client.cacheSprite[82].drawSprite(+xOffset, 0 + yOffset);
				...
				else
				{
						...
						else
						{
								...
								{
										Client.cacheSprite[Client.is474GameFrame ? 629 : 626].drawSprite(xOffset + (Client.is474GameFrame ? 12 : 24), Client.is474GameFrame ? 1 : 0);
								}
						}
				...
				if (Client.is474GameFrame && Client.isFixedScreen())
				{
						client.compass.method352(33, client.viewRotation, client.mapbackOffsets1, 245, client.mapbackOffsets0, 25, compassY, compassX - 1, 30, 25);
				}
				else
				{
						if (!Client.isFixedScreen())
						{
								client.compass.method352(33, client.viewRotation, client.mapbackOffsets1, 256, client.mapbackOffsets0, 24, compassY, compassX - 1, 33, 25);
						}
						else
						{
								client.compass.method352(33, client.viewRotation, client.mapbackOffsets1, 240, client.mapbackOffsets0, 25, compassY, compassX - 1, 33, 25);
						}
				}
		...
		public static void processMinimapActions(Client client)
		{
				...
				if (!Client.is474GameFrame && !Client.is474PlusGameFrame)
				{
						if ((Client.is562GameFrame || Client.is562PlusGameFrame) && x >= 745 && x <= 765 && y >= 0 && y <= 20)
						{
								client.menuActionName[1] = "Logout";
								client.menuActionID[1] = 1512;
								client.menuActionRow = 2;
						}
						else if (x >= (Client.is498GameFrame ? 743 : 745) && x <= 765 && y >= 0 && y <= (Client.is498GameFrame ? 25 : 21))
						{
								client.menuActionName[1] = "Logout";
								client.menuActionID[1] = 1512;
								client.menuActionRow = 2;
				...
				if (!Client.is474GameFrame || !Client.isFixedScreen())
				{
						if (Client.isFixedScreen() && x >= 523 && x <= 564 && y >= 3 && y <= 40 || !Client.isFixedScreen() && x >= 598 && x <= 635 && y >= 0 && y <= 40 && !Client.is474GameFrame || !Client.isFixedScreen() && Client.is474GameFrame && Client.inCircle(580, 0, x, y, 22))
						{
								client.menuActionName[1] = "Face North";
								client.menuActionID[1] = 477;
								client.menuActionRow = 2;
						}
						if (Client.is474PlusGameFrame || !Client.is474GameFrame)
						{

								if (Client.isFixedScreen() && x >= 706 && x <= 762 && y >= 95 && y <= 124 || !Client.isFixedScreen() && x >= 564 && x <= 621 && y >= 107 && y <= 140 && !Client.is474GameFrame || !Client.isFixedScreen() && Client.is474PlusGameFrame && x >= 580 && x <= 631 && y >= 130 && y <= 161)
								{
										client.menuActionName[1] = !Client.runClicked ? "Toggle Run-Mode On" : "Toggle Run-Mode Off";
										client.menuActionID[1] = 1050;
										client.menuActionName[0] = "Rest";
										client.menuActionID[0] = 1051;
										client.menuActionRow = 2;
								}
								else if (Client.isFixedScreen() && x >= 706 && x <= 762 && client.mouseY >= 54 && client.mouseY <= 86 || !Client.isFixedScreen() && x >= 554 && x <= 610 && y >= 72 && y <= 104 && !Client.is474GameFrame || !Client.isFixedScreen() && Client.is474PlusGameFrame && x >= 557 && x <= 612 && y >= 92 && y <= 126)
								{
										client.menuActionName[2] = Client.getQuickPrayerOn() ? "Turn Quick Prayers Off" : "Turn Quick Prayers On";
								...
								else if (Client.isFixedScreen() && x >= 690 && x <= 746 && client.mouseY >= 128 && client.mouseY <= 159 || !Client.isFixedScreen() && x >= 587 && x <= 643 && y >= 141 && y <= 172 && !Client.is474GameFrame || !Client.isFixedScreen() && Client.is474PlusGameFrame && x >= 615 && x <= 671 && y >= 159 && y <= 188)
								{
										if (!Client.summoningOrb)
										{
												return;
										}
				...
				else if (Client.is474PlusGameFrame)
				{
						...
						else if (Client.summoningOrb && x >= 530 && x <= 581 && client.mouseY >= 111 && client.mouseY <= 135 || !Client.summoningOrb && x >= 541 && x <= 594 && client.mouseY >= 122 && client.mouseY <= 153)
						{
								client.menuActionName[1] = !Client.runClicked ? "Toggle Run-Mode On" : "Toggle Run-Mode Off";
								client.menuActionID[1] = 1050;
								client.menuActionName[0] = "Rest";
						...
						else if (Client.summoningOrb && x >= 515 && x <= 569 && client.mouseY >= 78 && client.mouseY <= 110 || !Client.summoningOrb && x >= 515 && x <= 569 && client.mouseY >= 85 && client.mouseY <= 119)
						{
								client.menuActionName[2] = Client.getQuickPrayerOn() ? "Turn Quick Prayers Off" : "Turn Quick Prayers On";
								client.menuActionID[2] = 1500;
								client.menuActionRow = 2;
		...
		static void markMinimap(Client client, Sprite sprite, int xPosition, int yPosition)
		{
				...
				int xOffset = Client.isFixedScreen() ? 0 : Client.getClientWidth() - 211;
				int yOffset = Client.isFixedScreen() ? 0 : -4;
				...
				try
				{
						...
						sprite.drawSprite(((x + k1) - sprite.maxWidth / 2) + xOffset, (y - l1) - (sprite.maxHeight / 2) + yOffset);
				}
				catch (Exception _ex)
				{
				}
		...
		static void clickOnMiniMap()
		{
				...
				if (Client.instance.getClickMode3() == 1)
				{
						if (Client.is474GameFrame && Client.isFixedScreen())
						{
								...
								if ((Client.inCircle(0, 0, mouseX, mouseY, 73) && !(mouseX >= 21 && mouseX <= 48 && mouseY >= 132 && mouseY <= 141) && !(mouseX >= 21 && mouseX <= 43 && mouseY >= 124 && mouseY <= 131)) || mouseX >= 61 && mouseX <= 80 && mouseY >= 135 && mouseY <= 149)
								{
										mouseX -= 73;
										mouseY -= 75;
										...
										boolean flag1 = Client.instance.doWalkTo(1, 0, 0, 0, Client.myPlayer.smallY[0], 0, 0, j2, Client.myPlayer.smallX[0], true, i2);
										if (flag1)
										{
						...
						else
						{
								...
								if (!Client.isFixedScreen())
								{
										...
										x = Client.instance.saveClickX - 3 - (Client.getClientWidth() - 180);
										y += 4;
								}
		...
		public static void drawSplitPrivateChat()
		{
				...
				int y1 = Client.isFixedScreen() ? 0 : Client.getClientHeight() - 538;
				for (int j = 0; j < 100; j++)
				{
						if (Client.instance.chatMessages[j] != null)
						{
								...
								if ((k == 3 || k == 7) && (k == 7 || Client.instance.privateChatMode == 0 || Client.instance.privateChatMode == 1 && Client.instance.isFriendOrSelf(s)))
								{
										...
										Client.instance.newRegularFont.drawBasicString(s + ": " + Client.instance.chatMessages[j], x, y, 65535, 0);
										if (++i >= 5)
										{
												return;
										}
								}
								if (k == 5 && Client.instance.privateChatMode < 2)
								{
										int y = 329 - i * 13 + y1;
										Client.instance.newRegularFont.drawBasicString(Client.instance.chatMessages[j], 4, y, 65535, 0);
										if (++i >= 5)
										{
												return;
										}
								}
								if (k == 6 && Client.instance.privateChatMode < 2)
								{
										int y = 329 - i * 13 + y1;
										Client.instance.newRegularFont.drawBasicString("To " + s + ": " + Client.instance.chatMessages[j], 4, y, 65535, 0);
										if (++i >= 5)
										{
												return;
										}
