final class Rasterizer extends DrawingArea
{
		...
		public static void setupTextures()
		{
				if (texturePool == null)
				{
						...
						{
								texturePool[i] = new int[][] {new int[16384], new int[4096], new int[1024], new int[256], new int[64], new int[16], new int[4], new int[1]};
						}
						for (int k = 0; k < 51; k++)
								texturePixels[k] = null;
				}
		...
		{
				if (Client.smoothShading) // Using && drawTexturized causes potion white lines bug.
				{
		...
		private static void setMipmapLevel(int y1, int y2, int y3, int x1, int x2, int x3, int texture)
		{
				...
				// Daggannoth kings liar, the water texture.
				if (texture == 58)
				{
						setMipMapLevel(5);
						return;
				...
				// Venenatis webs on the floor.
				if (texture == 26 && Client.floorData.contains("606"))
				{
						setMipMapLevel(2);
						return;
				}
				...
				// 17 is water texture at Mage bank.
				// 34 is Magic tree.
				if (texture == 17 || texture == 34)
				{
						setMipMapLevel(0);
						return;
				}
		...
		public static Background textures[] = new Background[51];

		private static boolean[] textureIsTranslucent = new boolean[51];

		private static int[] textureRGB = new int[51];
		...
		public static int textureCycle[] = new int[51];

		public static int cycle;

		public static int palette[] = new int[0x10000];

		private static int[][] texturePalette = new int[51][];
