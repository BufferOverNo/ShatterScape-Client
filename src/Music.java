...
public class Music
{
				...
				for (int index = 0; index < Client.currentSound; index++)
				{
						...
						try
						{
								...
								if (stream != null)
								{
										...
										if (System.currentTimeMillis() + (long) (stream.currentOffset / 22) > Client.lastSoundTime + (long) (Client.lastSoundPosition / 22))
										{
												Client.lastSoundPosition = stream.currentOffset;
						...
						if (!flag1 || Client.soundDelay[index] == -5)
						{
								Client.currentSound--;
		...
		public static void packMusic()
		{
				for (int MusicIndex = 0; MusicIndex < 3536; MusicIndex++)
				{
						...
						{
								Client.instance.decompressors[3].method234(abyte0.length, abyte0, MusicIndex);
						}
				}
		}
		...
		public static byte[] packMusic(int Index)
		{
				try
				{
						File Music = new File(ClientConstants.MAIN_CACHE_LOCATION + "Music/" + Index + ".gz");
		...
		{
				if (ClientConfiguration.connorSettings)
				{
						return;
				}
		...
		private static int interfaceIdsToChangeColour[] = {5450, 6271, 6272, 9926, 3206};
