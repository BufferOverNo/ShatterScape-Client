final class SceneSpotAnim extends SceneNode
{
		...
		public Model getRotatedModel()
		{
				...
				int j = spotanim.aAnimation_407.primary[frame];
				int nextFrame1 = spotanim.aAnimation_407.primary[nextFrame];
				int cycle1 = spotanim.aAnimation_407.duration[frame];
				Model model_1 = new Model(true, SequenceFrame.isNegativeZero(j), false, model);
				if (!isFinished)
				{
						model_1.method469();
		...
		{
				for (cycle += i; cycle > spotanim.aAnimation_407.getDuration(frame);)
				{
						cycle -= spotanim.aAnimation_407.getDuration(frame) + 1;
						frame++;
						if (frame >= spotanim.aAnimation_407.length && (frame < 0 || frame >= spotanim.aAnimation_407.length))
						{
								frame = 0;
								isFinished = true;
						}
